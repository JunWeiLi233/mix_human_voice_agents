...
This generation command refuses Qwen verification evidence that launch readiness would reject, including wrong backend, wrong strategy, missing source details, missing or invalid verified WAV output, mismatched verified voice ids, or requested Qwen runtime options that disagree with the passed verification. Launch readiness also requires the generated Qwen mixed-voice `.wav` artifact to still exist alongside matching metadata.
...
The sequence validates that the manifest is a JSON object, that `voices` is an array, that `blend`, `agent_provider`, `generation`, `qwen`, and each `voices[]` item are objects when supplied, requires string voice identity/transcript/audio fields, requires optional voice notes to be strings, screens consent claims for public-figure or missing-permission language, requires at least two distinct speaker display names, checks each listed audio file exists, is a parseable WAV, contains audible signal, passes the same 5-30 second reference-audio quality gate used by voice import, rejects audio quality warnings such as clipping, requires any supplied voice `weight` to be positive, requires string `agent_provider.provider`, `agent_provider.model`, and `agent_provider.base_url` fields, requires optional provider `api_key` and `system_prompt` fields to be strings, requires a string `generation.prompt`, screens generation and provider test prompts for impersonation or fraud-like language, requires string launch blend fields and the strategy to be `multi_reference_prompt`, requires any supplied `qwen.text`, Qwen runtime option, and `agent_provider.prompt` to be strings and non-blank, and confirms `agent_provider.provider` is one of `openai`, `anthropic`, `google`, `xai`, `openai_compatible`, or `ollama` before importing anything. A normal run exits successfully only if the final launch-readiness audit is ready.
