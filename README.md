...
The sequence validates that the manifest is a JSON object, that `voices` is an array, that `blend`, `agent_provider`, `generation`, `qwen`, and each `voices[]` item are objects when supplied, requires string voice identity/transcript/audio fields, requires at least two distinct speaker display names, checks each listed audio file exists, is a parseable WAV, contains audible signal, requires any supplied voice `weight` to be positive, requires string `agent_provider.provider`, `agent_provider.model`, and `agent_provider.base_url` fields, requires a string `generation.prompt`, requires string launch blend fields and the strategy to be `multi_reference_prompt`, requires any supplied `qwen.text`, Qwen runtime option, and `agent_provider.prompt` to be strings and non-blank, and confirms `agent_provider.provider` is one of `openai`, `anthropic`, `google`, `xai`, `openai_compatible`, or `ollama` before importing anything. A normal run exits successfully only if the final launch-readiness audit is ready.
