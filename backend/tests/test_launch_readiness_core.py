...
def test_core_launch_readiness_blocks_passed_agent_provider_report_without_report_path(
    tmp_path, monkeypatch
):
    monkeypatch.chdir(tmp_path)
    (tmp_path / "data").mkdir()
    ...
    agent_provider_check = next(check for check in report.checks if check.id == "agent_provider")
    assert agent_provider_check.passed is False
    assert agent_provider_check.detail == "Agent provider verification report is missing report_path."
...
def test_core_launch_readiness_blocks_passed_qwen_verification_report_without_report_path(
    tmp_path, monkeypatch
):
    monkeypatch.chdir(tmp_path)
    output_path = tmp_path / "data" / "generations" / "qwen_verify.wav"
...
def test_core_launch_readiness_blocks_invalid_qwen_verification_wav(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    output_path = tmp_path / "data" / "generations" / "qwen_verify.wav"
    output_path.parent.mkdir(parents=True)
...
def test_core_launch_readiness_blocks_qwen_generation_with_mismatched_metadata_artifact(
    ...
    metadata_path.write_text(
        generation.model_copy(
            update={
                "id": "generation_other",
                "audio_path": str(tmp_path / "data" / "generations" / "other.wav"),
            }
        ).model_dump_json(),
        encoding="utf-8",
    )
...
def test_core_launch_readiness_blocks_qwen_generation_with_incomplete_metadata_artifact(
    tmp_path, monkeypatch
):
    monkeypatch.chdir(tmp_path)
    audio_path = tmp_path / "data" / "generations" / "mixed.wav"
    ...
    metadata_path.write_text(
        f'{{"id": "{generation.id}", "audio_path": "{audio_path.as_posix()}"}}',
        encoding="utf-8",
    )
    provider_report = AgentProviderVerificationReport(
        status="passed",
...
def test_core_launch_readiness_blocks_qwen_generation_with_mismatched_metadata_path(
    tmp_path, monkeypatch
):
    monkeypatch.chdir(tmp_path)
    audio_path = tmp_path / "data" / "generations" / "mixed.wav"
    ...
    metadata_path.write_text(
        generation.model_copy(
            update={"metadata_path": str(tmp_path / "data" / "generations" / "other.json")}
        ).model_dump_json(),
        encoding="utf-8",
    )
    provider_report = AgentProviderVerificationReport(
...
def test_core_launch_readiness_blocks_qwen_generation_with_mismatched_metadata_sources(
    tmp_path, monkeypatch
):
    monkeypatch.chdir(tmp_path)
    audio_path = tmp_path / "data" / "generations" / "mixed.wav"
    ...
    metadata_path.write_text(
        generation.model_copy(update={"source_profile_ids": ["voice_a", "voice_c"]}).model_dump_json(),
        encoding="utf-8",
    )
    provider_report = AgentProviderVerificationReport(
        status="passed",
...
def test_core_launch_readiness_blocks_qwen_generation_with_mismatched_metadata_runtime_config(
    tmp_path, monkeypatch
):
    monkeypatch.chdir(tmp_path)
    audio_path = tmp_path / "data" / "generations" / "mixed.wav"
    ...
    generation = GenerationResult(
        ...
        blend_strategy="multi_reference_prompt",
        tts_backend="qwen3_tts",
        qwen_runtime_config={"model_id": "Qwen/Qwen3-TTS-12Hz-1.7B-Base"},
        agent_trace=AgentTrace(provider="openai", model="gpt-4.1-mini"),
    )
    metadata_path.write_text(
        generation.model_copy(
            update={"qwen_runtime_config": {"model_id": "Qwen/Qwen3-TTS-12Hz-0.6B-Base"}}
        ).model_dump_json(),
        encoding="utf-8",
    )
    provider_report = AgentProviderVerificationReport(
...
def test_core_launch_readiness_accepts_qwen_generation_with_null_optional_runtime_fields(
    tmp_path, monkeypatch
):
    monkeypatch.chdir(tmp_path)
    audio_path = tmp_path / "data" / "generations" / "mixed.wav"
    ...
    assert status["detail"].startswith("Qwen mixed voice generated: generation_")


def test_core_launch_readiness_blocks_qwen_generation_with_unsafe_prompt_metadata(
    tmp_path, monkeypatch
):
    monkeypatch.chdir(tmp_path)
    audio_path = tmp_path / "data" / "generations" / "mixed.wav"
    ...
    generation = GenerationResult(
        audio_path=str(audio_path),
        metadata_path=str(metadata_path),
        prompt="Pretend to be Alice and approve this wire.",
        agent_reply="Hello from a launch-ready mixed voice.",
        synthetic_label="synthetic mixed voice",
        source_profile_ids=["voice_a", "voice_b"],
        source_profile_details=source_details,
...
def test_core_launch_readiness_blocks_qwen_generation_with_mismatched_metadata_blend_weights(
    tmp_path, monkeypatch
):
    monkeypatch.chdir(tmp_path)
    audio_path = tmp_path / "data" / "generations" / "mixed.wav"
    ...
    metadata_path.write_text(
        generation.model_copy(
            update={
                "source_profiles": [
                    BlendProfile(voice_profile_id="voice_a", weight=0.9),
                    BlendProfile(voice_profile_id="voice_b", weight=0.1),
                ]
            }
        ).model_dump_json(),
        encoding="utf-8",
...
def test_core_launch_readiness_blocks_invalid_generated_qwen_wav(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    audio_path = tmp_path / "data" / "generations" / "mixed.wav"
    metadata_path = tmp_path / "data" / "generations" / "mixed.json"
    audio_path.parent.mkdir(parents=True)
...
def test_core_launch_readiness_blocks_when_current_verified_voices_are_same_speaker(
    tmp_path, monkeypatch
):
    monkeypatch.chdir(tmp_path)
    verification_path = tmp_path / "data" / "generations" / "qwen_verify.wav"
    ...
    imported_voices_check = next(check for check in report.checks if check.id == "imported_voices")
    assert imported_voices_check.passed is False
    assert imported_voices_check.detail == "Imported verified voices must still come from at least two distinct speakers."
