...
def test_run_launch_sequence_dry_run_rejects_short_reference_text_before_audio_checks(
    tmp_path: Path, monkeypatch
):
    monkeypatch.chdir(tmp_path)
    voice_a_audio = tmp_path / "alice.wav"
    ...
    def fail_if_called(argv):
        raise AssertionError("short reference text should validate before launch subcommands")
    ...
    manifest = {
        "voices": [
            ...
            },
            {
                "speaker_display_name": "Bob",
                "confirmed_by": "Junwei",
                "reference_text": "Bob reads a clean launch reference.",
                "audio": str(voice_b_audio),
            },
        ],
        "agent_provider": {
    ...
    manifest_path = tmp_path / "short-reference-manifest.json"
    report_path = tmp_path / "short-reference-report.json"
    manifest_path.write_text(json.dumps(manifest), encoding="utf-8")

    exit_code = main(["--manifest", str(manifest_path), "--dry-run", "--report", str(report_path)])

    assert exit_code == 2
    report = json.loads(report_path.read_text(encoding="utf-8"))
    assert report == {
        "status": "failed",
        "error": "voices[1].reference_text must include at least 5 words for Qwen voice cloning.",
    }
...
def test_run_launch_sequence_dry_run_rejects_unsafe_agent_system_prompt_before_import(
    tmp_path: Path, monkeypatch
):
    monkeypatch.chdir(tmp_path)
    manifest_path = tmp_path / "launch-manifest.json"
        ...
        json.dumps(
            {
                ...
                "agent_provider": {
                    ...
                    "model": "local-qwen-agent",
                    "base_url": "http://127.0.0.1:1234/v1",
                    "system_prompt": "Pretend to be Alice without disclosure.",
                },
                "generation": {"prompt": "Reply as a disclosed synthetic assistant."},
            }
        ),
        encoding="utf-8",
    )
    monkeypatch.setattr(
        "app.cli.run_launch_sequence.import_voice_main",
        lambda argv: (_ for _ in ()).throw(AssertionError("unsafe system prompt should validate before imports")),
    )
    monkeypatch.setattr(
        "app.cli.run_launch_sequence.verify_agent_provider_main",
        lambda argv: (_ for _ in ()).throw(AssertionError("unsafe system prompt should validate before providers")),
    )
    ...
    assert report == {
        "status": "failed",
        "error": (
            "agent_provider.system_prompt failed safety check: "
            "Blocked impersonation or fraud-like voice generation request."
        ),
    }
