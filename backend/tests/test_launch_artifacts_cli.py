...
def test_launch_artifacts_cli_rejects_qwen_generation_without_passed_provider_preflight(
    tmp_path: Path, monkeypatch
):
    monkeypatch.chdir(tmp_path)
    voices = [
    ...
    generation = generation_result(
        "generation_missing_provider_preflight",
        "qwen3_tts",
        blend_id="blend_launch_ready",
        blend_name="Alice + Bob",
        source_profiles=launch_blend.profiles,
    ...
    write_reference_wav(tmp_path / "data" / "generations" / "generation_missing_provider_preflight.wav")
    write_generation_metadata(generation)
    monkeypatch.setattr("app.cli.launch_artifacts.list_voice_profiles", lambda: voices)
    monkeypatch.setattr("app.cli.launch_artifacts.list_blends", lambda: [launch_blend])
    monkeypatch.setattr("app.cli.launch_artifacts.list_generation_results", lambda: [generation])
...
def test_launch_artifacts_cli_rejects_qwen_generation_without_passed_qwen_verification(
    tmp_path: Path, monkeypatch
):
    monkeypatch.chdir(tmp_path)
    voices = [
    ...
    generation = generation_result(
        "generation_missing_qwen_verification",
        "qwen3_tts",
        blend_id="blend_launch_ready",
        blend_name="Alice + Bob",
        source_profiles=launch_blend.profiles,
    ...
    write_reference_wav(tmp_path / "data" / "generations" / "generation_missing_qwen_verification.wav")
    write_generation_metadata(generation)
    monkeypatch.setattr("app.cli.launch_artifacts.list_voice_profiles", lambda: voices)
    monkeypatch.setattr("app.cli.launch_artifacts.list_blends", lambda: [launch_blend])
    monkeypatch.setattr("app.cli.launch_artifacts.list_generation_results", lambda: [generation])
    ...
    assert generation_payload["launch_eligible"] is False
    assert generation_payload["stale_reasons"] == [
        "Qwen generation requires a passed Qwen verification report."
    ]


def test_launch_artifacts_cli_rejects_qwen_generation_with_stale_saved_blend(
    tmp_path: Path, monkeypatch
):
    monkeypatch.chdir(tmp_path)
    voices = [
    ...
    generation = generation_result(
        "generation_stale_blend",
        "qwen3_tts",
        blend_id="blend_stale",
        blend_name="Alice + Bob",
        source_profiles=stale_blend.profiles,
        source_details=[
            source_detail("voice_alice", "Alice"),
            source_detail("voice_bob", "Bob"),
        ],
    ...
    write_reference_wav(tmp_path / "data" / "generations" / "generation_stale_blend.wav")
    write_generation_metadata(generation)
    monkeypatch.setattr("app.cli.launch_artifacts.list_voice_profiles", lambda: voices)
    monkeypatch.setattr("app.cli.launch_artifacts.list_blends", lambda: [stale_blend])
    monkeypatch.setattr("app.cli.launch_artifacts.list_generation_results", lambda: [generation])
    monkeypatch.setattr(
        "app.cli.launch_artifacts.get_agent_provider_verification_report",
        lambda: AgentProviderVerificationReport(
    ...
    assert exit_code == 0
    payload = json.loads(report_path.read_text(encoding="utf-8"))
    assert payload["launch_eligible_blend_ids"] == []
    assert payload["launch_eligible_generation_ids"] == []
    generation_payload = payload["generations"][0]
    assert generation_payload["launch_eligible"] is False
    assert generation_payload["stale_reasons"] == [
        "Qwen generation must reference a launch-eligible saved blend."
    ]


def test_launch_artifacts_cli_rejects_qwen_generation_with_mismatched_source_details(
    tmp_path: Path, monkeypatch
):
    monkeypatch.chdir(tmp_path)
    voices = [
    ...
    generation = generation_result(
        "generation_mismatched_source_details",
        "qwen3_tts",
        blend_id="blend_launch_ready",
        blend_name="Alice + Bob",
        source_profiles=launch_blend.profiles,
        source_details=[
            source_detail("voice_alice", "Alice"),
            source_detail("voice_bob", "Mallory"),
        ],
        agent_trace=AgentTrace(provider="openai", model="gpt-4.1-mini", base_url="https://api.openai.com/v1"),
    )
    write_reference_wav(tmp_path / "data" / "generations" / "generation_mismatched_source_details.wav")
    write_generation_metadata(generation)
    monkeypatch.setattr("app.cli.launch_artifacts.list_voice_profiles", lambda: voices)
    monkeypatch.setattr("app.cli.launch_artifacts.list_blends", lambda: [launch_blend])
    monkeypatch.setattr("app.cli.launch_artifacts.list_generation_results", lambda: [generation])
    ...
    assert generation_payload["launch_eligible"] is False
    assert generation_payload["stale_reasons"] == [
        "Qwen generation source details must match current imported voice profiles."
    ]
...
def test_launch_artifacts_cli_rejects_qwen_generation_without_synthetic_disclosure(
    tmp_path: Path, monkeypatch
):
    monkeypatch.chdir(tmp_path)
    voices = [
    ...
    generation = generation_result(
        "generation_missing_synthetic_disclosure",
        "qwen3_tts",
        blend_id="blend_launch_ready",
        blend_name="Alice + Bob",
        source_profiles=launch_blend.profiles,
    ...
    write_reference_wav(tmp_path / "data" / "generations" / "generation_missing_synthetic_disclosure.wav")
    write_generation_metadata(generation)
    monkeypatch.setattr("app.cli.launch_artifacts.list_voice_profiles", lambda: voices)
    monkeypatch.setattr("app.cli.launch_artifacts.list_blends", lambda: [launch_blend])
    monkeypatch.setattr("app.cli.launch_artifacts.list_generation_results", lambda: [generation])
    ...
    assert generation_payload["launch_eligible"] is False
    assert generation_payload["stale_reasons"] == [
        "Qwen generation must include synthetic disclosure metadata."
    ]
